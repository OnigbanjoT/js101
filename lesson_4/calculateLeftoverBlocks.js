...
function calculateLeftoverBlocks(startingBlocks) {
  let remainingBlocks = startingBlocks;
  let currentLayer = 0;

  let nextLayer = currentLayer + 1;
  let numberOfBlocksRequired = nextLayer * nextLayer;

  while (remainingBlocks >= numberOfBlocksRequired) {
    remainingBlocks -= numberOfBlocksRequired;
    currentLayer += 1;
  }
